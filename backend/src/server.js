...
import { ensureCsrfCookie, validateCsrfToken } from './middleware/csrfMiddleware.js';
...
app.use(helmet());
app.use(cors({ origin: process.env.FRONTEND_URL?.split(',').map((url) => url.trim()), credentials: true }));
